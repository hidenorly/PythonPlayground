...
class ModifiedGitChecker:
	def extract_git_old_new(git_path, tmp_path, branches, file_extensions, file_in_regexp=None, file_out_regexp=None):
		if len(branches)!=2:
			if not branches[0]:
				branches[0]="HEAD"
			branches = [GitUtil.get_tail(git_path), branches[0]]
		changed_files = GitUtil.changed_files(git_path, branches[0], branches[1], file_extensions, file_in_regexp, file_out_regexp)
	...
	def ensure_git_clone(git_path, tmp_clone_path):
