...
class MethodDef:
    name: str
    number: Optional[int]  # Cap'n only
    input_type: str
    output_type: str
...
class CapnpParser:
    ...
    def parse(text: str) -> Schema:
        text = CapnpParser.strip(text)
        schema = Schema()

        # enums
        for m in CapnpParser.ENUM_RE.finditer(text):
            name, body = m.groups()
            values = {}
            for v in CapnpParser.ENUM_VALUE_RE.finditer(body):
                vname, num = v.groups()
                num = int(num)
        ...
        # structs
        for m in CapnpParser.STRUCT_RE.finditer(text):
            name, body = m.groups()
            fields = {}
            for f in CapnpParser.STRUCT_FIELD_RE.finditer(body):
                fname, num, ftype = f.groups()
                num = int(num)
                fields[num] = FieldDef(fname, num, ftype)
            schema.messages[name] = MessageDef(name, fields)

        # interfaces
        for m in CapnpParser.INTERFACE_RE.finditer(text):
            name, body = m.groups()
            methods = {}
            for mm in CapnpParser.METHOD_RE.finditer(body):
                mname, num, _, _ = mm.groups()
...
class ProtoParser:
    ...
    @staticmethod
    def parse(text: str) -> Schema:
        text = ProtoParser.strip(text)
        schema = Schema()

        # enums
        for m in ProtoParser.ENUM_RE.finditer(text):
            name, body = m.groups()
            values = {}
            for v in ProtoParser.ENUM_VALUE_RE.finditer(body):
                vname, num = v.groups()
                num = int(num)
                values[num] = EnumValue(vname, num)
            schema.enums[name] = EnumDef(name, values)
        ...
        # messages
        for m in ProtoParser.MESSAGE_RE.finditer(text):
            name, body = m.groups()
            fields = {}
            for f in ProtoParser.FIELD_RE.finditer(body):
        ...
        # services
        for m in ProtoParser.SERVICE_RE.finditer(text):
            name, body = m.groups()
            methods = {}
            for r in ProtoParser.RPC_RE.finditer(body):
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: api_check.py OLD NEW")
        sys.exit(1)
